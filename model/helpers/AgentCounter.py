...
class AgentCounter(Helper, object):
    ...
    def step_epilogue(self, model):
        cancerCells = len([a for a in model.schedule.agents if
                           a.__class__.__name__ == self.cancerCellClassName])
        tipCells = len([a for a in model.schedule.agents if
                        a.__class__.__name__ == self.tipCellClassName or
                        a.__class__.__name__ == self.trunkCellName])
        aliveCancerCells = len([a for a in model.schedule.agents if
                                a.__class__.__name__ ==
                                self.cancerCellClassName and not a.dead])
        deadCancerCells = len([a for a in model.schedule.agents if
                               a.__class__.__name__ ==
                               self.cancerCellClassName and a.dead])

...
class GlucoseDiffusionHelper(object, Helper):
    def __init__(self, model, cancerCellName="CancerCell"):
        ...
        self.glucoseDiffusionCoeff = model.properties["diffusion"]["glucoseDiffusivity"]
        ...
        self.minGlucoseUptakeRate = model.properties["agents"]["cancerCells"]["minGlucoseUptakeRate"]
        self.maxGlucoseUptakeRate = model.properties["agents"]["cancerCells"]["maxGlucoseUptakeRate"]
    ...
    def getSourceSinkGrids_(self, phi, model):
        ...
        sinkGrid = CellVariable(name="sink", mesh=Grid3D(dx=1, dy=1, dz=1, nx=xsize, ny=ysize, nz=zsize))
        ...
        for coordinate, agents in agentGrid.items():
            ...
            # Getting the current sink rate, defined as the sum of the sink rates of all non-dead and non-quiescent
            # cancer cells and healthy cells at this position
            sinkWarburg = [a.glucoseUptakeRate for a in agents if a.__class__.__name__ == self.cancerCellName
                           and not (a.dead or a.quiescent) and a.warburgSwitch]
            sinkRateWarburg = sum(sinkWarburg)

            sinkNonWarburg = [a.glucoseUptakeRate for a in agents if (a.__class__.__name__ == self.cancerCellName
                                                                      and not (
                                a.dead or a.quiescent) and not a.warburgSwitch) or (a.__class__.__name__ == "HealthyCell" and not a.dead)]

            sinkRateNonWarburg = sum(sinkNonWarburg)

            # Getting the current source rate, defined as the sum of source rates of all Tip and Trunk cells at
            # this position
            sourceRate = sum(
                [a.glucoseSecretionRate for a in agents if a.__class__.__name__ in ("TipCell", "TrunkCell")])
            ...
            estimatedConcentration = concentrationAtPos + sourceRate - sinkRateWarburg - sinkRateNonWarburg

            # If our estimated concentration is greater than our source rate, this means we really are outputting
            ...
            if estimatedConcentration >= self.baseGlucoseSecretionRate:
                sourceRate -= min(sourceRate, estimatedConcentration - self.baseGlucoseSecretionRate)

            # If our estimate concentration is below zero, then our sinks should be reduced. We reduce them by the
            # magnitude of the negative value, but of course we can't reduce them beyond the original value.
            ...
            if estimatedConcentration < 0:
                ...
                ratioNonWarburg = numNonWarburg / totSink if totSink > 0 else 0.

                # Sink rates cannot be lower than respective minimum glucose uptake rates, otherwise this means
                # we don't have enough glucose and the cell should die. Each sink will be decreased by the proportion
                # of negative estimate concentration for which they are responsible
                sinkRateWarburg -= min(sinkRateWarburg - self.maxGlucoseUptakeRate * ratioWarburg,
                                       abs(estimatedConcentration) * ratioWarburg)
                sinkRateNonWarburg -= min(sinkRateNonWarburg - self.minGlucoseUptakeRate * ratioNonWarburg,
    ...
    def stepPrologue(self, model):
        ...
        while not suitableSolution:
            ...
            if iteration > 2:
                print("Glucose diffusion still has negative positions at epochs %s despite killing all agents at such coordinates..." % str(model.currentEpoch))
        ...
        for x in range(nx):
            for y in range(ny):
                for z in range(nz):
                    model.environments[self.glucoseEnvName].grid[(x, y, z)] = cs[x][y][z]

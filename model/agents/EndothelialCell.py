...
class EndothelialCell(Agent, object):
    ...
    def __init__(self, model, radius=1):
        ...
        if radius < 1:
            raise Exception('Trying to instanstiate endothelial cell with invalid radius %s' % str(radius))
...
class TipCell(EndothelialCell, object):
    def __init__(self, model, radius=1):
        ...
        endothelialCellsProperties = model.properties["agents"]["endothelialCells"]
        self.minimumVegfConcentration = endothelialCellsProperties["minimumVegfConcentration"]
    ...
    def stepMain(self, model):
        ...
        currentPosition = self.environmentPositions[self.agentEnv]

        tafAtCurrentPosition = model.environments[self.vegfEnv].grid[currentPosition]

        if tafAtCurrentPosition >= self.minimumVegfConcentration and self.cellAge == self.divisionDelay and 10.*random() < tafAtCurrentPosition:
            self.cellAge = 0

            neigh = model.environments[self.agentEnv].getMooreNeighbourhood(self.environmentPositions[self.agentEnv])

            # get taf concentration at each neighbouring position
            neighToTaf = [(n, model.environments[self.vegfEnv].grid[n]) for n in neigh]
            ...
            while model.environments[self.agentEnv].grid[newPos].__len__() >= model.properties["maxAgentDensity"]:
    ...
    def stepMainOld(self, model):
        ...
        if tafAtCurrentPosition >= self.minimumVegfConcentration and self.cellAge == self.divisionDelay:
            self.cellAge = 0

            # If we are not sprouting
            if not self.decideSproutLinear_(tafAtCurrentPosition):

                # Getting next position
                scoredCandidates = self.rankNeighbours_(model)
                nextPosition = self.getNextNeighFromScored_(scoredCandidates)


                if model.environments[self.agentEnv].grid[nextPosition].__len__() < model.properties["maxAgentDensity"]:
                    # Move self to next position
                    self.MoveAgent(self.agentEnv, nextPosition, model)

                    # Create tip cell at old position1
                    t = TrunkCell(model,radius=self.radius)
                    t.AddAgentToGrid(self.agentEnv, currentPosition, model)
                    model.schedule.agentsToSchedule.add(t)
            # Else, if we are sprouting
            else:
                for _ in range(2):
                    t = TipCell(model,radius=self.radius-1)
                    t.AddAgentToGrid(self.agentEnv, currentPosition, model)
                    model.schedule.agentsToSchedule.add(t)

                self.RemoveAgent(model)
    ...
    # Given a list of sorted neighbours where each takes the form (coords, score), returns a neighbour
    # where those with a higher score have the highest probability of being picked
    ...
    # Assigns a score to each neighbourhood position where 0 = awful and 1 = best and sum(scores) = 1
    def rankNeighbours_(self, model):
        ...
        # In the case where there is no taf, we treat all neighbouring positions alike
        if totalTaf < 0.0001:
            scoredNeigh = [(x[0], 0) for x in neighToTaf]
        else:
            # scoring the neighbourhood
            scoredNeigh = [(x[0], float(x[1]) / float(totalTaf)) for x in neighToTaf]

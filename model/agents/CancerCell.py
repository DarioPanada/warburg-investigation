...
class CancerCell(Agent, object):
    ...
    def stepMain(self, model):
        ...
        self.glucoseAtPos = model.environments[self.glucoseEnvName].grid[currentPos]
        self.drugAtPos = model.environments[self.drugEnvName].grid[currentPos]


        '''# If a cell is quiescent and the drug dropped, we have it return active
        ...
        if not self.dead and not self.quiescent:
            ...
            if random.random() < self.pWarburgSwitch and not self.warburgSwitch:
                self.warburgSwitch = True
                self.glucoseUptakeRate = self.maxGlucoseUptakeRate
            # If the cell is not warburg, then in order for it to survive it must:
                # Have access to minimum oxygen concentration
                # Have access to minimum glucose for non warburg cells
            # If the cell is a warburg cell then, in order for it to survive, it must:
                # Have access to minimum glucose for warburg cells
            if not self.decideDie_():

                # If a cell is a warburg cell and the dosage at position is greater than the minimum threshold,
                # it becomes quiescent. (We return so that no further cell logic is executed)

                # TEMPORARILY DISABLING DRUGS, THIS SHOULD BE TURNED INTO A FLAG THAT CAN BE SET FROM PARAMS

                '''if self.drugAtPos >= self.minDrugDosage and self.warburgSwitch:
                    self.quiescent = True
                    return'''
    ...
    def updateHifExpressionRate_(self, model):
        ...
        if not self.warburgSwitch:
            ...
            newRate = self.calculateHifExpressionRateFromOxygenWarburg_(self.oxygenAtPos)
        ...
        if currentRate > newRate:
            newRate = max(0, currentRate - min(maxShiftPerEpoch, currentRate - newRate))
        else:
            newRate = min(currentRate + min(maxShiftPerEpoch, newRate - currentRate), 16)
    ...
    def calculateHifExpressionRateFromOxygenWarburg_(self, oxygenAtPos):
        ...
        if oxygenAtPos > self.oxygenUltraHypoxicDomain:
            ...
            domain = [self.oxygenUltraHypoxicDomain,self.oxygenWarburgHypoxicDomain]
        else:
            coef = self.oxygenToHifCoeffsUltraHypoxic
            domain = [0.0, self.oxygenUltraHypoxicDomain]
